...
const Promotions = () => {
    return (
        <main className="container mx-auto px-4 py-6 space-y-12">
            {/* Hero Banner */}
            <section className="relative overflow-hidden rounded-xl bg-slate-900 aspect-[21/9] flex items-center">
                <img alt="Tech Banner" className="absolute inset-0 w-full h-full object-cover opacity-60" src="https://lh3.googleusercontent.com/aida-public/AB6AXuCmGX7w4pNiPGqL2RKR5UUqgixQQ434FA7bprXFVKURJtmwGsdw2Kcm6hDjKz2h2b23zJ09o2a1lNzcwVBIETToTF-7Ssa69HKN0Of5TatrbM1p1eNZzYQzfeEvDBbd4HxatYQcZD9HI2fyCBDg39bFWjKbvOZCotkcw82H4hbGvZMQVWSZkZlbFFP4KHpqiSdtXtnTjtenmnoZNvR3UWlqEEP24k-ostQ9mALR5QhHTYXwLf9ro7fq0VxbSa48NDP_sdvi7p_C7kM" />
                <div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/60 to-transparent"></div>
                <div className="relative z-10 px-12 max-w-2xl space-y-6">
                    <span className="inline-block px-3 py-1 bg-primary text-white text-xs font-bold uppercase tracking-widest rounded-full">Sự kiện lớn nhất năm</span>
                    <h1 className="text-5xl lg:text-7xl font-extrabold text-white leading-tight">Tech Season <br /><span className="text-primary">Sale - Up to 50%</span></h1>
                    <p className="text-slate-300 text-lg">Nâng cấp thiết bị của bạn với mức giá chưa từng có. Cam kết chính hãng 100%.</p>
                    <div className="flex gap-4">
                        <button className="px-8 py-3 bg-primary text-white font-bold rounded-lg hover:bg-primary/90 transition-all transform hover:scale-105">Săn ngay</button>
                        <button className="px-8 py-3 bg-white/10 text-white backdrop-blur-md font-bold rounded-lg hover:bg-white/20 transition-all">Xem chi tiết</button>
                    </div>
                </div>
            </section>
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 text-red-500">
                            ...
                            <h2 className="text-2xl font-extrabold uppercase italic">Flash Sale</h2>
                        </div>
                        <div className="flex items-center gap-2 ml-4">
                            <span className="text-slate-500 font-medium text-sm">Kết thúc sau:</span>
                            <div className="flex gap-1.5">
                                <span className="bg-slate-800 text-white px-2 py-1 rounded font-bold text-lg">00</span>
                                <span className="text-slate-800 dark:text-white font-bold text-lg">:</span>
                                <span className="bg-slate-800 text-white px-2 py-1 rounded font-bold text-lg">15</span>
                                <span className="text-slate-800 dark:text-white font-bold text-lg">:</span>
                                <span className="bg-primary text-white px-2 py-1 rounded font-bold text-lg animate-pulse">30</span>
                            </div>
                        </div>
                    </div>
                    <Link to="/products" className="text-primary font-bold hover:underline text-sm flex items-center">Xem tất cả <span className="material-icons text-sm">chevron_right</span></Link>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    {[
                        ...
                        <div key={idx} className="group cursor-pointer">
                            <div className="relative aspect-square bg-slate-50 dark:bg-slate-800 rounded-lg overflow-hidden mb-3">
                                ...
                                <div className="absolute top-2 left-2 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded">{item.discount}</div>
                            </div>
                            <h3 className="font-medium text-sm line-clamp-2 mb-1 group-hover:text-primary">{item.name}</h3>
                            <div className="flex items-end gap-2 mb-3">
                                <span className="text-primary font-bold text-lg">{item.price}</span>
                                <span className="text-slate-400 text-xs line-through mb-1">{item.oldPrice}</span>
                            </div>
                            <div className="space-y-1">
                                <div className="h-4 w-full bg-slate-100 dark:bg-slate-700 rounded-full relative overflow-hidden">
                                    ...
                                    <span className="absolute inset-0 flex items-center justify-center text-[10px] text-white font-bold uppercase">{typeof item.sold === 'number' ? `Đã bán ${item.sold}` : item.sold}</span>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        ...
                        <div key={idx} className={`bg-white dark:bg-slate-900 border-2 border-dashed border-primary/30 rounded-xl p-4 flex items-center gap-4 relative ${voucher.disabled ? 'opacity-60' : ''}`}>
                            <div className={`w-16 h-16 ${voucher.disabled ? 'bg-slate-200 dark:bg-slate-800 text-slate-500 border-slate-300' : 'bg-primary/10 text-primary border-primary/20'} rounded-lg flex flex-col items-center justify-center border flex-shrink-0`}>
                                {voucher.icon ? <span className="material-icons">{voucher.icon}</span> : <span className="text-xs font-bold uppercase">{voucher.type}</span>}
                                <span className={`text-${voucher.icon ? '[10px]' : 'lg'} font-black italic`}>{voucher.value}</span>
                            </div>
                            <div className="flex-grow">
                                <h4 className="font-bold text-sm">{voucher.title}</h4>
                                <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-2">{voucher.desc}</p>
                                <div className="flex items-center justify-between">
                                    <code className={`bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded ${voucher.disabled ? 'text-slate-400 line-through' : 'text-primary'} font-mono text-xs font-bold`}>{voucher.code}</code>
                                    <button className={`text-[10px] ${voucher.disabled ? 'bg-slate-400 cursor-not-allowed' : 'bg-primary hover:bg-primary/90'} text-white font-bold px-3 py-1.5 rounded uppercase`}>{voucher.disabled ? 'Đã hết' : 'Sao chép'}</button>
                                </div>
                            </div>
                        </div>
                    ))}
             ...
             <section className="space-y-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <h2 className="text-2xl font-bold">Săn deal theo danh mục</h2>
                    <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                        <button className="px-6 py-2 bg-white dark:bg-slate-700 shadow-sm rounded-lg text-primary font-bold text-sm transition-all">Smartphone</button>
                        <button className="px-6 py-2 text-slate-500 font-bold text-sm hover:text-primary transition-all">Laptop</button>
                        <button className="px-6 py-2 text-slate-500 font-bold text-sm hover:text-primary transition-all">Phụ kiện</button>
                        <button className="px-6 py-2 text-slate-500 font-bold text-sm hover:text-primary transition-all">Đồ gia dụng</button>
                    </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        ...
                        <div key={idx} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 hover:shadow-xl transition-shadow group relative">
                            <div className="absolute top-4 right-4 z-10 w-8 h-8 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center shadow-md text-slate-400 hover:text-red-500 cursor-pointer transition-colors">
                                <span className="material-icons text-xl">favorite_border</span>
                            </div>
                            <div className="aspect-[4/3] bg-slate-50 dark:bg-slate-800 rounded-lg overflow-hidden mb-4">
                                <img alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform" src={item.img} />
                            </div>
                            <div className="space-y-2">
                                <div className="flex items-center gap-1">
                                    <span className="material-icons text-orange-400 text-sm">star</span>
                                    <span className="text-xs font-bold">{item.rating}</span>
                                    <span className="text-xs text-slate-500">({item.reviews} đánh giá)</span>
                                </div>
                                <h3 className="font-bold text-slate-900 dark:text-white line-clamp-2 min-h-[3rem]">{item.name}</h3>
                                <div className="flex flex-col">
                                    <span className="text-primary font-extrabold text-xl">{item.price}</span>
                                    <span className="text-slate-400 text-sm line-through">{item.oldPrice}</span>
                                </div>
                                <button className="w-full mt-4 py-2 border-2 border-primary text-primary font-bold rounded-lg group-hover:bg-primary group-hover:text-white transition-all">Thêm vào giỏ</button>
                            </div>
                        </div>
                    ))}
                </div>
                <div className="flex justify-center">
                    <button className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 px-8 py-3 rounded-xl font-bold hover:bg-slate-200 transition-colors">
